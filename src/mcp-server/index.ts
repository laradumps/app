...
server.registerPrompt(
    ...
    async () => {
        ...
        if (!lastError) {
            return {
                messages: [{ role: 'user', content: { type: 'text', text: 'No recent error logs found to analyze.' } }]
            };
        }
...
server.registerPrompt('summarize_logs', { description: 'Summarize the recent application logs' }, async () => {
    const data = await fetchData('logs');
    if (data.error) {
        return {
            messages: [{ role: 'user', content: { type: 'text', text: `Failed to fetch logs: ${data.error}` } }]
        ...
        messages: [
            {
                ...
                content: {
                    type: 'text',
                    text: `Please provide a summary of the following application logs:\n\n${JSON.stringify(logs, null, 2)}`
                }
            }
        ]
    };
...
server.registerPrompt(
    ...
    async () => {
        ...
        if (data.error) {
            return {
                messages: [{ role: 'user', content: { type: 'text', text: `Failed to fetch queries: ${data.error}` } }]
            };
        }
